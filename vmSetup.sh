...
vboxmanage storageattach $1 --storagectl "IDE controller" --device 0 --port 1 --type dvddrive
...
VBoxManage unattended install $1 --iso=$2 --hostname=$1.ws2-2223-ube.hogent --user=ladmin --password=Admin2021 --install-additions --additions-iso=$3 --full-user-name=lAdmin --country=BE --image-index=$4 --start-vm=gui --post-install-command="shutdown /r /t 0"
}
function unattendedInstallTest() {
VBoxManage unattended install $1 --iso=$2 --hostname=$1.ws2-2223-ube.hogent --user=ladmin --password=Admin2021 --full-user-name=lAdmin --country=BE --image-index=$3 --start-vm=gui --post-install-command="powershell copy-Item -Path E: -Destination C:\scripts -Recurse && powershell invoke-expression C:\scripts\master.ps1"
}
...
for i in $*; do 
    ...
    dc)
        ...
        unattendedInstall "dc" $windowsServerIso $guestAdditionsIso 1
        ;;
